...
Useful to generate kinematogram stimuli from optic flow components extracted in matlab
(cf. generate_crcns_hyperflow.m)
...
PATH = "/home/ulysse/PycharmProjects/NRFdatasets/CRCNS_MT1"  # "/mnt/Data1/CRCNS_MT1"
    ...
    for frame in range(fields.shape[-1]):
        ...
        if frame == 0:
            ...
            if len(vidx) > 0:
                ...
                star_pos = np.zeros((ndots, 2), dtype=float)
        ...
        star_pos[:, 0] += (
            speed_mult * xv[ypos.astype(int) % sz, xpos.astype(int) % sz]
        )
        star_pos[:, 1] += (
            speed_mult * yv[ypos.astype(int) % sz, xpos.astype(int) % sz]
        )
        ...
        dotr = dotr[validx].astype(int)
        dotc = dotc[validx].astype(int)
...
def generate_unmatched_sequence(f, stem):
    ...
    Y_hf = Y_hf.T   # transpose

    """
    assert Xidx_hf.shape[0] == Y_hf.shape[0]
    print(os.path.join(PATH, f"movies/{stem}.h5"))
    fout = tables.open_file(os.path.join(PATH, f"movies/{stem}.h5"), "w")
    fout.create_array("/", "Xidx_hf", obj=Xidx_hf)  # TODO: shape ?
    fout.create_array("/", "X_hf", obj=X_hf)        # (T, H, W)
    fout.create_array("/", "Y_hf", obj=Y_hf)        # (t<T,)
    fout.close()
    """
