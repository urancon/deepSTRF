...
# Load the recording
datafile = os.path.join(signals_dir, 'data_files_here/PEG_NAT4_ozgf.fs100.ch18.tgz')
...
"""# Save sets
est.save(os.path.join(signals_dir, 'recordings/peg_est/'))
val.save(os.path.join(signals_dir, 'recordings/peg_val/'))"""
...
#responses2 = torch.index_select(responses1, dim=1, index=good_idx)
val_responses = torch.permute(responses1, (1,0,2))

list_neurons = pd.read_csv("data_files_here/PEG_pred_corr.csv")
...
responses_tensor = responses_tensor.view(398, 575, 150)
est_responses = responses_tensor.to(dtype=torch.float32)

ccmax = torch.load('ccmax_peg.pt')
# ccmax = torch.mean(ccmax,dim=1)

auditory = good_idx

torch.save({'est_spectrograms': est_spectrograms, 'est_responses': est_responses, 'val_spectrograms': val_spectrograms, 'val_responses': val_responses, 'ccmax': ccmax, 'auditory':auditory}, "../nat4_peg.pt")
