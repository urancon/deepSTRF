...
if __name__ == "__main__":
    ...
    from metrics.metrics import compute_CCmax, compute_CCmax2, compute_TTRC
    dataset = NS1_DRC_Dataset('./NS1_DRC/data/', stimuli=('nat', 'drc'))
    dataset.select_neuron(17)
    from torch.utils.data import DataLoader
    loader = DataLoader(dataset, batch_size=1, shuffle=False)
    ulysse_ccmax = []
    corentin_ccmax = []
    for i, (spectro, responses, psth, ccmax) in enumerate(loader):
        #print(i, spectro.shape, responses.shape, psth.shape, ccmax.shape)
        print(f"{i})", ccmax.item(), compute_CCmax(responses, 126).item())
        corentin_ccmax.append(ccmax.item())
        #ulysse_ccmax.append(torch.sqrt(compute_TTRC(responses)).item())
        ulysse_ccmax.append(compute_CCmax(responses, 126).item())

        '''
        plt.figure()
        plt.plot(psth.squeeze(), label='mean+hanning')
        plt.plot(responses.squeeze().mean(0), label='mean')
        #plt.plot(responses[0][:10].mean(dim=0))
        #plt.plot(responses[0][10:].mean(dim=0))
        plt.legend()
        plt.show()
        '''
    ...
    plt.figure()
    plt.scatter(corentin_ccmax[:20], ulysse_ccmax[:20], label='nat')
    plt.scatter(corentin_ccmax[20:32], ulysse_ccmax[20:32], label='drc')
    plt.plot([0, 1], [0, 1], color='red', label='y=x')
    plt.xlabel('corentin ccmax')
    plt.ylabel('ulysse ccmax')
    plt.legend()
    plt.show()
    print("ok")
