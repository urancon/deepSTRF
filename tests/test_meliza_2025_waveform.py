"""Tests for the raw-waveform branch of the Meliza 2025 dataset.
...
_MELIZA_LOCAL = os.environ.get(
    "MELIZA2025_DATA",
    "/home/ulysse/Documents/NRFdatasets/Audio/ZF_Restore_Bao/zebf-auditory-restoration-1",
)
_HAS_DATA = os.path.isdir(os.path.join(_MELIZA_LOCAL, "nat8b-stimuli"))


@pytest.mark.skipif(not _HAS_DATA,
                    reason=f"Meliza2025 data not at {_MELIZA_LOCAL!r}; set $MELIZA2025_DATA.")
@pytest.mark.skipif(_mem_available_gb() < 18.0,
                    reason="nat8b loads ~14 GB of responses; need >=18 GB free to avoid OOM.")
def test_meliza_waveform_branch():
    """Meliza's waveform branch hands out the 48 kHz source wavs grid-locked to
    ...
    ds = Meliza2025Dataset(path=_MELIZA_LOCAL, experiment="nat8b",
    ...
    for s in range(ds.get_S()):
        ...
        assert stim.dim() == 2 and stim.shape[0] == 1, \
            f"Meliza stim {s} must be (1, T_audio); got {tuple(stim.shape)}"
