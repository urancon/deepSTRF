...
def test_ns1_waveform_shape_and_metadata():
    ...
    ``audio_fs`` and ``T_audio = 999 * 80`` at the default 16 kHz."""
    from deepSTRF.datasets.audio.ns1_drc import NS1Dataset

    ds = NS1Dataset(return_waveform=True, audio_fs=16000)
    assert ds.audio_fs == 16000
    assert ds.get_S() == 20
    assert len(ds.stims) == 20
    for s in range(20):
        assert ds.stims[s].shape == (1, 999 * 80), \
...
def test_ns1_responses_identical_between_modes():
    ...
    ds_spec = NS1Dataset()
    ds_wav = NS1Dataset(return_waveform=True, audio_fs=16000)
