...
ccraw_array = []
ccnorm_array = []
ccnorm_allseeds_array = np.zeros((len(seeds), dataset.n))
...
for neuron_index in tqdm(range(dataset.n)):
    ...
    for seed, i in enumerate(seeds):
        ...
        ccnorm_allseeds_array[i, neuron_index] = test_cc_norm
...
ccnorm_allseeds_array = np.mean(ccnorm_allseeds_array, 1)
